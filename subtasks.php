...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['subtask'])) {
    ...
    $stmt->execute([$task_id, $subtask, $due_date, $priority, 0]);

    $_SESSION['success_message'] = "Subtugas berhasil ditambahkan!";
...
if (isset($_GET['delete_subtask'])) {
    ...
    $stmt = $pdo->prepare("SELECT task_id FROM subtasks WHERE id = ?");
...
if (isset($_POST['mark_all_completed'])) {
    ...
    $_SESSION['success_message'] = "Semua subtugas ditandai selesai!";
    header("Location: subtasks.php?task_id=$task_id");
    exit();
}
?>
            ...
            <?php foreach ($subtasks as $subtask): ?>
                <tr>
                    ...
                    <td class="action-buttons">
                        ...
                        <!-- Link Hapus Subtugas -->
                        <a href="subtasks.php?task_id=<?= $task_id ?>&delete_subtask=<?= $subtask['id']; ?>" class="delete-btn"
   onclick="return confirm('Hapus subtugas ini?')">
   🗑️ Hapus
</a> 
                    </td>
                </tr>
